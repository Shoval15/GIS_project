...
import json
import time
from Services import heat
import os

DEBUGGING = False
IMPORT_CACHE = True
DIR = 'forPresentation2'
FULL_PATH_DIR = os.path.join(r".\Services\myCache", DIR)
ALGORITHM = 'knapsack'
...
def get_bounds():
    ...
    if IMPORT_CACHE:   
        buildings_gdf = gpd.read_file(os.path.join(FULL_PATH_DIR ,"buildings.geojson"))
        renewal_gdf = gpd.read_file(os.path.join(FULL_PATH_DIR , "renewal.geojson"))
        gardens_gdf = gpd.read_file(os.path.join(FULL_PATH_DIR , "gardens.geojson"))
    else:
        buildings_gdf = import_data.import_buildings(bounds_polygon, polygon)        
        buildings_gdf.to_file(os.path.join(FULL_PATH_DIR ,"buildings.geojson"), driver="GeoJSON")
        ...
        gardens_gdf.to_file(os.path.join(FULL_PATH_DIR , "gardens.geojson"), driver="GeoJSON")

        if gardens_gdf.empty :
            return jsonify({"status": "failed", "received": data, "response": "importLayer"})
            
        renewal_gdf = import_data.import_urban_renewal(bounds_polygon, polygon)
        renewal_gdf.to_file(os.path.join(FULL_PATH_DIR , "renewal.geojson"), driver="GeoJSON")
